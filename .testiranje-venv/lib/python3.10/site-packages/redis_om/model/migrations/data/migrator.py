...
class DataMigrator:
    ...
    def discover_migrations(self) -> Dict[str, BaseMigration]:
        ...
        if not self._discovered_migrations:
            if self.migrations_dir:
                self._load_migrations_from_directory(self.migrations_dir)
            elif self.migration_module:
                self._load_migrations_from_module(self.migration_module)
    ...
    def _load_migrations_from_directory(self, migrations_dir: str) -> None:
    ...
    def _load_migrations_from_module(self, module_name: str) -> None:
    ...
    def mark_migration_unapplied(self, migration_id: str) -> None:
    ...
    def run_migrations(
        ...
        for migration in pending_migrations:
            ...
            try:
                migration.up()
                self.mark_migration_applied(migration.migration_id)
                applied_count += 1
    ...
    def run_migrations_with_monitoring(
        self,
        dry_run: bool = False,
    ...
    def rollback_migration(
        ...
        try:
            migration.down()
            self.mark_migration_unapplied(migration_id)
